...
static int tc_attach(const char *ifname, const char *obj_path, const char *direction)
{
    char cmd[512];
    ...
    snprintf(cmd, sizeof(cmd), "tc filter add dev %s %s bpf da obj %s sec tc", ifname, direction, obj_path);
...
{
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <interface> [direction] [ef_mbps af_mbps be_mbps]\n", argv[0]);
        fprintf(stderr, "  direction: ingress or egress (default: egress)\n");
        fprintf(stderr, "Example: %s ens19 egress 500 300 200\n", argv[0]);
        fprintf(stderr, "         %s ens19 ingress 500 300 200\n", argv[0]);
        return 1;
    }
    ...
    /* Parse direction (default: egress) */
    const char *direction = "egress";
    int rate_arg_start = 2;
    ...
    __u64 rates[TC_MAX];
    rates[TC_EF] = argc > rate_arg_start     ? atoi(argv[rate_arg_start])     : 500;
    rates[TC_AF] = argc > rate_arg_start + 1 ? atoi(argv[rate_arg_start + 1]) : 300;
    rates[TC_BE] = argc > rate_arg_start + 2 ? atoi(argv[rate_arg_start + 2]) : 200;
    ...
    /* Attach eBPF */
    if (tc_attach(ifname, obj_path, direction) != 0)
        return 1;
